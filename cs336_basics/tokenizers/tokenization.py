...
def pretokenize_chunked(num_processes: int, input_path: str, special_tokens: list[str]) -> dict[tuple[bytes], int]:
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as pool:
        results = list(
            pool.map(
                pretokenize_chunk,
                [[input_path, boundaries[i], boundaries[i + 1], st_split_pattern] for i in range(len(boundaries) - 1)],
            )
        )
    combined = Counter()
...
def train_bpe(
    ...
    if num_processes == 1:
        ...
        pretokens = pretokenize_chunked(
            num_processes=num_processes, input_path=input_path, special_tokens=special_tokens
        )

    vocab, merge_list = merge(pretokens=pretokens, new_vocab_size=vocab_size - len(special_tokens))
